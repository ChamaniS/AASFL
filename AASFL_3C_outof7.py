...
dataDir = "C:/Users/csj5/Projects/Data/BlastocystDATA/7Clients/Nonuniform/"
        ...
        if (len(R_cls) == 0):
            ...
            for R_round in (range(args.roundsR)):
                ...
                if (round_idxR < args.roundsR):
                    ...
                    C3M1_LBR.update((x, y * D2R) for x, y in C3M1_LBR.items())
                    C3M2_LBR.update((x, y * D2R) for x, y in C3M2_LBR.items())
                    C3M3_LBR.update((x, y * D2R) for x, y in C3M3_LBR.items())
                    C4M1_LBR.update((x, y * D2R) for x, y in C4M1_LBR.items())
                    C4M2_LBR.update((x, y * D2R) for x, y in C4M2_LBR.items())
                    C4M3_LBR.update((x, y * D2R) for x, y in C4M3_LBR.items())


                    M1dictR = [C1M1_LBR, C2M1_LBR,C3M1_LBR,C4M1_LBR]
                    M2dictR = [C1M2_LBR, C2M2_LBR,C3M2_LBR,C4M2_LBR]
            ...
            for NR_round in (range(args.roundsNR)):
                ...
                for cl_idx in NR_cls:
                    ...
                    if (round_idxNR < args.roundsNR):
                        ...
                        M1dictNR = [C5M1_LBNR, C6M1_LBNR, C7M1_LBNR]
                        M2dictNR = [C5M2_LBNR, C6M2_LBNR, C7M2_LBNR]
                        M3dictNR = [C5M3_LBNR, C6M3_LBNR, C7M3_LBNR]
