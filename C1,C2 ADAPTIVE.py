...
def main():
    ...
    #client 1
    local_ep1 = math.ceil((((Tmax / s1time) + D1) + S_C1) * E)
    print("C1 local epochs:", local_ep1)

    local_ep2 = math.ceil((((Tmax / s2time) + D2) + S_C2) * E)
    print("C2 local epochs:", local_ep2)

    local_ep3 = math.ceil((((Tmax / s3time) + D3) + S_C3) * E)
    print("C3 local epochs:", local_ep3)

    local_ep4 = math.ceil((((Tmax / s4time) + D4) + S_C4) * E)
    print("C4 local epochs:", local_ep4)

    local_ep5 = math.ceil((((Tmax / s5time) + D5) + S_C5) * E)
    print("C5 local epochs:", local_ep5)

    #define the LR per each client

    lr_C1 = (Tmax / s1time) * (S_C1/2*D1) *10*args.fixed_lr;
    lr_C2 = (Tmax / s2time) * (S_C2/2*D2) *10*args.fixed_lr;
    lr_C3 = (Tmax / s3time) * (S_C3/2*D3) *10*args.fixed_lr;
    lr_C4 = (Tmax / s4time) * (S_C4/2*D4) *10*args.fixed_lr;
    lr_C5 = (Tmax / s5time) * (S_C5/2*D5) *10*args.fixed_lr;
    ...
    for com_round in (range(args.comrounds)):
            ...
            for R_round in (range(args.roundsR)):
                ...
                for cl_idx in R_cls:
                    ...
                    if cl_idx == 4:
                        ...
                        scheduler1 = ExponentialLR(C4m1opR, gamma=0.9)
                        scheduler2 = ExponentialLR(C4m2opR, gamma=0.9)
                        scheduler3 = ExponentialLR(C4m3opR, gamma=0.9)
                    if cl_idx == 5:
                        train_loader = train_loader_C5
                        val_loader = val_loader_C5
                        ...
                        scheduler1 = ExponentialLR(C5m1opR, gamma=0.9)
                        scheduler2 = ExponentialLR(C5m2opR, gamma=0.9)
                        scheduler3 = ExponentialLR(C5m3opR, gamma=0.9)
                    ...
                    if cl_idx in R_cls:
                        ...
                        if cl_idx == 4:  # C4--------------------------------------------------------------C4 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep4):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1R, local_model2R, local_model3R, C4m1opR, C4m2opR,C4m3opR, scheduler1, scheduler2, scheduler3, loss_fn, C4_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        ...
                        if cl_idx == 5:  # C5--------------------------------------------------------------C5 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep5):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1R, local_model2R, local_model3R, C5m1opR, C5m2opR,C5m3opR, scheduler1, scheduler2, scheduler3, loss_fn, C5_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                                    val_loader, local_model1R, local_model2R, local_model3R, loss_fn, folder,
                                    C5_EPL)
                ...
                if (round_idxR < args.roundsR):
                    ...
                    M1dictR = [C3M1_LBR,C4M1_LBR,C5M1_LBR]
                    M2dictR = [C3M2_LBR,C4M2_LBR,C5M2_LBR]
                    M3dictR = [C3M3_LBR,C4M3_LBR,C5M3_LBR]
            ...
            for NR_round in (range(args.roundsNR)):
                ...
                for cl_idx in NR_cls:
                    ...
                    if cl_idx == 1:
                        ...
                        scheduler1 = ExponentialLR(C1m1opNR, gamma=0.9)
                        scheduler2 = ExponentialLR(C1m2opNR, gamma=0.9)
                        scheduler3 = ExponentialLR(C1m3opNR, gamma=0.9)
                    elif cl_idx == 2:
                        train_loader = train_loader_C2
                        val_loader = val_loader_C2
                        ...
                        scheduler1 = ExponentialLR(C2m1opNR, gamma=0.9)
                        scheduler2 = ExponentialLR(C2m2opNR, gamma=0.9)
                        scheduler3 = ExponentialLR(C2m3opNR, gamma=0.9)
                    ...
                    if (round_idxNR < args.roundsNR):
                        ...
                        C1M1_LBNR.update((x, y * WC4) for x, y in C1M1_LBNR.items())
                        C1M2_LBNR.update((x, y * WC4) for x, y in C1M2_LBNR.items())
                        C1M3_LBNR.update((x, y * WC4) for x, y in C1M3_LBNR.items())
                        C2M1_LBNR.update((x, y * WC5) for x, y in C2M1_LBNR.items())
                        C2M2_LBNR.update((x, y * WC5) for x, y in C2M2_LBNR.items())
                        C2M3_LBNR.update((x, y * WC5) for x, y in C2M3_LBNR.items())


                        M1dictNR = [C1M1_LBNR, C2M1_LBNR]
                        M2dictNR = [C1M2_LBNR, C2M2_LBNR]
                        M3dictNR = [C1M3_LBNR, C2M3_LBNR]
        ...
        if least_lossg > avg_g_val_loss:  #good
