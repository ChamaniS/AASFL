...
from errorcorrection_main.adaptiveFL_utils_splitfed_shallow_kvasir import (get_loaders, eval, get_loaders_test, test)
...
dataDir="C:/Users/csj5/Projects/Data/kvasir-seg/federated/kvasir-seg/Kvasir-SEG"
parentF="C:/Users/csj5/Projects/Adaptive_Splitfed-V3/"
#client 1
...
def train_screen(train_loader, local_model1, local_model2, local_model3, optimizer1, optimizer2, optimizer3, loss_fn, PL):
    ...
    return epoch_loss, epoch_acc, epoch_iou_withbackground, epoch_iou_nobackground, epoch_iou_class0, epoch_iou_class1, isreliableclient, quantified_PL
...
def main():
    ...
    for idx in range(5):
        ...
        for epoch in range(args.screen_ep):
            ...
            if cl_idx == 1:  # C1---------------------------------------------------------------C1 screen training & validation--------------------------------------------------------------------------------------------------------------------
                ...
                S_train_epoch_loss, S_train_epoch_acc, S_trainepoch_iou_withbackground, S_trainepoch_iou_nobackground, S_trainepoch_iou_class0, S_trainepoch_iou_class1, isreliable1, quantified_PL = train_screen(
                ...
                S_val_epoch_loss, S_val_epoch_acc, S_valepoch_iou_withbackground, S_valepoch_iou_nobackground, S_valepoch_iou_class0, S_valepoch_iou_class1 = eval(
            ...
            print("T: S_Tumor:", S_trainepoch_iou_class1)
            ...
            print("V: S_Tumor:", S_valepoch_iou_class1)
    ...
    print("C3 screening time:", s3time)
    print("C4 screening time:", s4time)
    print("C5 screening time:", s5time)

    stime=[]
    stime.extend([s1time, s2time, s3time,s4time,s5time])
    Tmax = max(stime)
    ...
    mul1 = (Tmax / s1time)*D1*S_C1*n
    mul2 = (Tmax / s2time) * D2 * S_C2*n
    mul3 = (Tmax / s3time)*D3*S_C3*n
    mul4 = (Tmax / s4time) * D4 * S_C4*n
    mul5 = (Tmax / s5time) * D5 * S_C5*n

    if 1 in R_cls:
        testd1 = R_maxLR/mul1
    elif 1 in NR_cls:
        testd1 = NR_maxLR / mul1
    if 2 in R_cls:
        testd2 = R_maxLR/mul2
    elif 2 in NR_cls:
        testd2 = NR_maxLR / mul2
    if 3 in R_cls:
        testd3 = R_maxLR/mul3
    elif 3 in NR_cls:
        testd3 = NR_maxLR / mul3
    if 4 in R_cls:
        testd4 = R_maxLR/mul4
    elif 4 in NR_cls:
        testd4 = NR_maxLR / mul4
    if 5 in R_cls:
        testd5 = R_maxLR/mul5
    elif 5 in NR_cls:
        testd5 = NR_maxLR / mul5

    testarr = [testd1, testd2, testd3,testd4,testd5]
    testdmin = min(testarr)

    if (E1<12):
        lr_C1 = testdmin*(Tmax / s1time)*D1*S_C1*(E/E1)*n
    else:
        lr_C1 = testdmin*(Tmax / s1time)*D1*S_C1*n
    if (E2<12):
        lr_C2 = testdmin * (Tmax / s2time) * D2 * S_C2*(E/E2)*n
    else:
        lr_C2 = testdmin * (Tmax / s2time) * D2 * S_C2*n
    if (E3<12):
        lr_C3 = testdmin*(Tmax / s3time)*D3*S_C3*(E/E3)*n
    else:
        lr_C3 = testdmin * (Tmax / s3time) * D3 * S_C3* n
    if (E4<12):
        lr_C4 = testdmin * (Tmax / s4time) * D4 * S_C4*(E/E4)*n
    else:
        lr_C4 = testdmin * (Tmax / s4time) * D4 * S_C4*n
    if (E5<12):
        lr_C5 = testdmin * (Tmax / s5time) * D5 * S_C5*(E/E5)*n
    else:
        lr_C5 = testdmin * (Tmax / s5time) * D5 * S_C5 * n
            ...
            for R_round in (range(args.roundsR)):
                ...
                for cl_idx in R_cls:
                    ...
                    if cl_idx == 1:
                        ...
                        scheduler1 = ExponentialLR(C1m1opR, gamma=0.9)
                        scheduler2 = ExponentialLR(C1m2opR, gamma=0.9)
                        scheduler3 = ExponentialLR(C1m3opR, gamma=0.9)
                    elif cl_idx == 2:
                        train_loader = train_loader_C2
                        val_loader = val_loader_C2
                        ...
                        scheduler1 = ExponentialLR(C2m1opR, gamma=0.9)
                        scheduler2 = ExponentialLR(C2m2opR, gamma=0.9)
                        scheduler3 = ExponentialLR(C2m3opR, gamma=0.9)
                    elif cl_idx == 3:
                        train_loader = train_loader_C3
                        val_loader = val_loader_C3
                        ...
                        scheduler1 = ExponentialLR(C3m1opR, gamma=0.9)
                        scheduler2 = ExponentialLR(C3m2opR, gamma=0.9)
                        scheduler3 = ExponentialLR(C3m3opR, gamma=0.9)
                    ...
                    if cl_idx in R_cls:
                        ...
                        if cl_idx == 2:  # C2--------------------------------------------------------------C2 local training & validation--------------------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep2):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1= train(
                                    train_loader, local_model1R, local_model2R, local_model3R, C2m1opR, C2m2opR,C2m3opR,scheduler1,scheduler2,scheduler3, loss_fn, C2_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1 = eval(
                        ...
                        if cl_idx == 3:  # C3--------------------------------------------------------------C3 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep3):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1 = train(
                                    train_loader, local_model1R, local_model2R, local_model3R, C3m1opR, C3m2opR,C3m3opR,scheduler1,scheduler2,scheduler3, loss_fn, C3_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1 = eval(
            ...
            for NR_round in (range(args.roundsNR)):
                ...
                for cl_idx in NR_cls:
                    ...
                    if cl_idx == 4:
                        ...
                        scheduler1 = ExponentialLR(C4m1opNR, gamma=0.9)
                        scheduler2 = ExponentialLR(C4m2opNR, gamma=0.9)
                        scheduler3 = ExponentialLR(C4m3opNR, gamma=0.9)
                    if cl_idx == 5:
                        train_loader = train_loader_C5
                        val_loader = val_loader_C5
                        ...
                        scheduler1 = ExponentialLR(C5m1opNR, gamma=0.9)
                        scheduler2 = ExponentialLR(C5m2opNR, gamma=0.9)
                        scheduler3 = ExponentialLR(C5m3opNR, gamma=0.9)
                    ...
                    if (round_idxNR < args.roundsNR):
                        ...
                        M1dictNR = [C4M1_LBNR, C5M1_LBNR]
                        M2dictNR = [C4M2_LBNR, C5M2_LBNR]
                        M3dictNR = [ C4M3_LBNR, C5M3_LBNR]
