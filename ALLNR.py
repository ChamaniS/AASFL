...
def main():
    ...
    tot_loader = len(train_loader_C1) + len(train_loader_C2) + len(train_loader_C3) + len(train_loader_C4) + len(
        train_loader_C5) + len(train_loader_C6) + len(train_loader_C7)
    ...
    D6NR = len(train_loader_C6) / sum(lenNR)
    D7NR = len(train_loader_C7) / sum(lenNR)
    ...
    C6hsoft = np.exp(C6h)
    C7hsoft = np.exp(C7h)
    softC = C1hsoft + C2hsoft + C3hsoft + C4hsoft + C5hsoft + C6hsoft + C7hsoft;
    ...
    for com_round in (range(args.comrounds)):
        ...
        if (len(NR_cls) == 0):
            ...
            for NR_round in (range(args.roundsNR)):
                ...
                least_lossC1NR, least_lossC2NR, least_lossC3NR, least_lossC4NR, least_lossC5NR, least_lossC6NR, least_lossC7NR = 100000000, 100000000, 100000000, 100000000, 100000000, 100000000, 100000000;
                ...
                for cl_idx in NR_cls:
                    ...
                    if cl_idx == 6:
                        ...
                        C6m1opNR = optim.Adam(local_model1NR.parameters(), lr=lr_C6)
                        C6m2opNR = optim.Adam(local_model2NR.parameters(), lr=lr_C6)
                        C6m3opNR = optim.Adam(local_model3NR.parameters(), lr=lr_C6)
                        scheduler1 = ExponentialLR(C6m1opNR, gamma=1.0)
                        scheduler2 = ExponentialLR(C6m2opNR, gamma=1.0)
                        scheduler3 = ExponentialLR(C6m3opNR, gamma=1.0)
                    if cl_idx == 7:
                        train_loader = train_loader_C7
                        val_loader = val_loader_C7
                        folder = parentF + "./Outmain/noisycom/Fed_Avg/Saved/local_models/client7"
                        C7m1opNR = optim.Adam(local_model1NR.parameters(), lr=lr_C7)
                        C7m2opNR = optim.Adam(local_model2NR.parameters(), lr=lr_C7)
                        C7m3opNR = optim.Adam(local_model3NR.parameters(), lr=lr_C7)
                        scheduler1 = ExponentialLR(C7m1opNR, gamma=1.0)
                        scheduler2 = ExponentialLR(C7m2opNR, gamma=1.0)
                        scheduler3 = ExponentialLR(C7m3opNR, gamma=1.0)
                    ...
                    if cl_idx in NR_cls:
                        if cl_idx == 1:  # C1-------------------------------------------------------------C1 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep1):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1NR, local_model2NR, local_model3NR, C1m1opNR, C1m2opNR,
                                    C1m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C1_PL)
                                print("Client", cl_idx, "local validating.........")
                        ...
                        if cl_idx == 2:  # C2-------------------------------------------------------------C2 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep2):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1NR, local_model2NR, local_model3NR, C2m1opNR, C2m2opNR,
                                    C2m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C2_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        ...
                        if cl_idx == 3:  # C3--------------------------------------------------------------C3 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep3):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1NR, local_model2NR, local_model3NR, C3m1opNR, C3m2opNR,
                                    C3m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C3_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        ...
                        if cl_idx == 4:  # C4--------------------------------------------------------------C4 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep4):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    ...
                                    C4m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C4_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        ...
                        if cl_idx == 5:  # C5--------------------------------------------------------------C5 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep5):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    ...
                                    C5m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C5_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        ...
                        if cl_idx == 6:  # C6--------------------------------------------------------------C6 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep6):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1NR, local_model2NR, local_model3NR, C6m1opNR, C6m2opNR,
                                    C6m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C6_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                                    val_loader, local_model1NR, local_model2NR, local_model3NR, loss_fn, folder, C6_EPL)
                                ...
                                if least_lossC6NR > val_epoch_loss:
                                    least_lossC6NR = val_epoch_loss
                                    torch.save(local_model1NR.state_dict(),
                                               parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M1_localcheckpointNR.pth')
                                    torch.save(local_model2NR.state_dict(),
                                               parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M2_localcheckpointNR.pth')
                                    torch.save(local_model3NR.state_dict(),
                                               parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M3_localcheckpointNR.pth')
                                    print('C6localmodel saved')
                        ...
                        if cl_idx == 7:  # C7--------------------------------------------------------------C7 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep7):
                                ...
                                train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                                    train_loader, local_model1NR, local_model2NR, local_model3NR, C7m1opNR, C7m2opNR,
                                    C7m3opNR, scheduler1, scheduler2, scheduler3, loss_fn, C7_PL)
                                print("Client", cl_idx, "local validating.........")
                                val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                                    val_loader, local_model1NR, local_model2NR, local_model3NR, loss_fn, folder, C7_EPL)
                                ...
                                if least_lossC7NR > val_epoch_loss:
                                    least_lossC7NR = val_epoch_loss
                                    torch.save(local_model1NR.state_dict(),
                                               parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M1_localcheckpointNR.pth')
                                    torch.save(local_model2NR.state_dict(),
                                               parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M2_localcheckpointNR.pth')
                                    torch.save(local_model3NR.state_dict(),
                                               parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M3_localcheckpointNR.pth')
                                    print('C7localmodel saved')
                    ...
                    if (round_idxNR < args.roundsNR):
                        C1M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C1M1_localcheckpointNR.pth')
                        C1M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C1M2_localcheckpointNR.pth')
                        C1M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C1M3_localcheckpointNR.pth')
                        C2M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C2M1_localcheckpointNR.pth')
                        C2M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C2M2_localcheckpointNR.pth')
                        C2M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C2M3_localcheckpointNR.pth')
                        C3M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C3M1_localcheckpointNR.pth')
                        C3M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C3M2_localcheckpointNR.pth')
                        C3M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C3M3_localcheckpointNR.pth')
                        C4M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C4M1_localcheckpointNR.pth')
                        C4M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C4M2_localcheckpointNR.pth')
                        C4M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C4M3_localcheckpointNR.pth')
                        C5M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C5M1_localcheckpointNR.pth')
                        C5M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C5M2_localcheckpointNR.pth')
                        C5M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C5M3_localcheckpointNR.pth')
                        C6M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M1_localcheckpointNR.pth')
                        C6M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M2_localcheckpointNR.pth')
                        C6M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M3_localcheckpointNR.pth')
                        C7M1_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M1_localcheckpointNR.pth')
                        C7M2_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M2_localcheckpointNR.pth')
                        C7M3_LBNR = torch.load(
                            parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M3_localcheckpointNR.pth')
                        ...
                        C6M1_LBNR.update((x, y * WC6) for x, y in C6M1_LBNR.items())
                        C6M2_LBNR.update((x, y * WC6) for x, y in C6M2_LBNR.items())
                        C6M3_LBNR.update((x, y * WC6) for x, y in C6M3_LBNR.items())
                        C7M1_LBNR.update((x, y * WC7) for x, y in C7M1_LBNR.items())
                        C7M2_LBNR.update((x, y * WC7) for x, y in C7M2_LBNR.items())
                        C7M3_LBNR.update((x, y * WC7) for x, y in C7M3_LBNR.items())
                        M1dictNR = [C1M1_LBNR,C2M1_LBNR,C3M1_LBNR, C4M1_LBNR, C5M1_LBNR, C6M1_LBNR, C7M1_LBNR]
                        M2dictNR = [C1M2_LBNR,C2M2_LBNR,C3M2_LBNR, C4M2_LBNR, C5M2_LBNR, C6M2_LBNR, C7M2_LBNR]
                        M3dictNR = [C1M3_LBNR,C2M3_LBNR,C3M3_LBNR, C4M3_LBNR, C5M3_LBNR, C6M3_LBNR, C7M3_LBNR]
        ...
        C6M1 = torch.load(parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M1_localcheckpointNR.pth')
        C6M2 = torch.load(parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M2_localcheckpointNR.pth')
        C6M3 = torch.load(parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C6M3_localcheckpointNR.pth')
        C7M1 = torch.load(parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M1_localcheckpointNR.pth')
        C7M2 = torch.load(parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M2_localcheckpointNR.pth')
        C7M3 = torch.load(parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C7M3_localcheckpointNR.pth')
