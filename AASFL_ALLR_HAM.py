...
def main():
    ...
    if (E1<12):
        lr_C1 = (testdmin*(T1*(1/D1)+S_C1))*(E/E1)*n
    else:
        lr_C1 =(testdmin*(T1*(1/D1)+S_C1))*n
    if (E2<12):
        lr_C2 = (testdmin*(T2*(1/D2)+S_C2))*(E/E2)*n
    else:
        lr_C2 =(testdmin*(T2*(1/D2)+S_C2))*n
    if (E3<12):
        lr_C3 =(testdmin*(T3*(1/D3)+S_C3))*(E/E3)*n
    else:
        lr_C3 = (testdmin*(T3*(1/D3)+S_C3))*n
    if (E4<12):
        lr_C4 = (testdmin*(T4*(1/D4)+S_C4))*(E/E4)*n
    else:
        lr_C4 = (testdmin*(T4*(1/D4)+S_C4))*n
    if (E5<12):
        lr_C5 = (testdmin*(T5*(1/D5)+S_C5))*(E/E5)*n
    else:
        lr_C5 = (testdmin*(T5*(1/D5)+S_C5))*n
        ...
        if (len(R_cls) == 0):
            ...
            for R_round in (range(args.roundsR)):
                        ...
                        if cl_idx == 4:  # C4--------------------------------------------------------------C4 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep4):
                                ...
                                if least_lossC4R > val_epoch_loss:
                                    ...
                                    torch.save(local_model1R.state_dict(),parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C4M1_localcheckpointR.pth')
                                    torch.save(local_model2R.state_dict(),parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C4M2_localcheckpointR.pth')
                                    torch.save(local_model3R.state_dict(),parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C4M3_localcheckpointR.pth')
                                    print('C4localmodel saved')
                        ...
                        if cl_idx == 5:  # C5--------------------------------------------------------------C5 local training & validation-----------------------------------------------------------------------------------------------------------
                            for epoch in range(local_ep5):
                                ...
                                if least_lossC5R > val_epoch_loss:
                                    ...
                                    torch.save(local_model1R.state_dict(),parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C5M1_localcheckpointR.pth')
                                    torch.save(local_model2R.state_dict(),parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C5M2_localcheckpointR.pth')
                                    torch.save(local_model3R.state_dict(),parentF + './Outmain/noisycom/Fed_Avg/Checkpoints/C5M3_localcheckpointR.pth')
                                    print('C5localmodel saved')
                ...
                if (round_idxR < args.roundsR):
                    ...
                    M1dictR = [C1M1_LBR, C2M1_LBR,C3M1_LBR,C4M1_LBR,C5M1_LBR]
                    M2dictR = [C1M2_LBR, C2M2_LBR,C3M2_LBR,C4M2_LBR,C5M2_LBR]
                    M3dictR = [C1M3_LBR, C2M3_LBR,C3M3_LBR, C4M3_LBR,C5M3_LBR]
